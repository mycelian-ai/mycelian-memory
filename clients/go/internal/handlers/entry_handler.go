...
func (eh *EntryHandler) handleAddEntry(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	start := time.Now()
	ack, err := eh.client.AddEntryInVault(ctx, userID, vaultID, memoryID, client.AddEntryRequest{
		RawEntry: rawEntry,
		Summary:  summary,
		Tags:     tags,
	})
...
func (eh *EntryHandler) handleListEntries(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	start := time.Now()
	resp, err := eh.client.ListEntriesInVault(ctx, userID, vaultID, memoryID, params)
	elapsed := time.Since(start)
