...
class SessionSimulator:
    """Drive a single benchmark session through Claude with Synapse tool calls.
    ...
    short-term context; long-term context is stored in Synapse.
    """

    def __init__(
        self,
        anthropic_client,
        synapse_client: SynapseMemoryClient,
        ...
        *,
        ...
        # without issuing a second round-trip to the Synapse CLI / HTTP API.
    ...
    async def step(self, user_text: str, max_tokens: int = 2000) -> str:
        ...
        rate_backoff = 15.0      # HTTP 429 – start with 15 s (TPM bucket)
        ...
        while True:
            ...
            # ----------------------------------------------------------
                ...
                rate_backoff = min(rate_backoff * 2, 300)  # cap at 5 min
                rate_attempt += 1
                continue
    ...
    async def _exec_create_memory(self, args: Dict[str, Any]) -> None:
        ...
        title = args["title"]
        memory_type = args.get("memory_type", "chat")
