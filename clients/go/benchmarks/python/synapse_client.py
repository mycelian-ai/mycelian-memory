...
class SynapseMemoryClient:
    """Thin HTTP wrapper around Synapse MCP service used by the benchmark harness.

    Only the subset of endpoints required for benchmark ingestion & evaluation
    is implemented.  All methods raise ``requests.HTTPError`` on non-2xx.
    """

    def __init__(self, base_url: str, user_id: str | None = None, session: Optional[requests.Session] = None):
        """Create client.

        If *user_id* is None, a fresh user is created automatically via the
        `synapse` CLI, and the returned UUID is stored for subsequent calls.
        """
        self.base_url = base_url.rstrip("/")
        # Allow caller to inject a custom session (e.g. for retries or mocks)
        self._session = session or requests.Session()

        if user_id:
            self.user_id = user_id
        else:
            self.user_id = self._get_or_create_cached_user()



    def _get_or_create_cached_user(self) -> str:
        """Always create a fresh benchmark user via CLI (no file cache)."""
    ...
    def create_memory(self, title: str, memory_type: str = "chat", description: str = "") -> str:
        ...
        try:
            # ------------------------------------------------------------------
            # Preferred path – CLI (keeps parity with other tools/tests)
            # ------------------------------------------------------------------
            out = self._run_cli(
                "create-memory",
                "--user-id",
            ...
            # ------------------------------------------------------------------
            logging.warning("[create_memory] CLI failed – falling back to HTTP: %s", cli_err)
            payload = {
            ...
            resp = self._session.post(
                f"{self.base_url}/api/users/{self.user_id}/memories",
                json=payload,
                timeout=30,
            )
            try:
                ...
                raise RuntimeError(
                    f"create_memory failed via CLI and HTTP fallback: {http_err}"
                ) from http_err
            data = resp.json() or {}
            mem_id = (
                data.get("memoryId")
            ...
            if not mem_id:
                raise RuntimeError(f"create_memory: unable to parse memoryId from response: {data}")
            return mem_id
        # Prefer CLI to create memory to avoid schema drift.
        out = self._run_cli(
            "create-memory",
            "--user-id",
            self.user_id,
    ...
    def get_context(self, memory_id: str) -> str:
        """Return latest context document using CLI; fallback to HTTP on error."""
