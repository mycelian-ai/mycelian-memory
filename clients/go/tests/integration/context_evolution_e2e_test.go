...
func TestContextEvolutionE2E(t *testing.T) {
	...
	mem, err := c.CreateMemoryInVault(ctx, user.ID, vault.VaultID, client.CreateMemoryRequest{Title: "EvoTest", MemoryType: "NOTES"})
	if err != nil {
	...
	for i, snap := range snapshots {
		if _, err := c.PutMemoryContext(ctx, user.ID, vault.VaultID, mem.ID, snap); err != nil {
			t.Fatalf("put context %d: %v", i, err)
		}
		raw := fmt.Sprintf("entry-%d", i+1)
		if _, err := c.AddEntryInVault(ctx, user.ID, vault.VaultID, mem.ID, client.AddEntryRequest{RawEntry: raw, Summary: "evo"}); err != nil {
	...
	latestCtx, err := c.GetLatestMemoryContext(ctx, user.ID, vault.VaultID, mem.ID)
	if err != nil {
...
func TestMultiAgentContextAccessE2E(t *testing.T) {
	...
	mem, err := agentA.CreateMemoryInVault(ctx, user.ID, vault.VaultID, client.CreateMemoryRequest{Title: "MATest", MemoryType: "NOTES"})
	if err != nil {
		t.Fatalf("create memory: %v", err)
	}

	originalCtx := "Agent A context"
	if _, err := agentA.PutMemoryContext(ctx, user.ID, vault.VaultID, mem.ID, originalCtx); err != nil {
		t.Fatalf("agentA put context: %v", err)
	}
	_ = agentA.AwaitConsistency(ctx, mem.ID)

	if _, err := agentA.AddEntryInVault(ctx, user.ID, vault.VaultID, mem.ID, client.AddEntryRequest{RawEntry: "A entry", Summary: "sum"}); err != nil {
	...
	resCtx, err := agentB.GetLatestMemoryContext(ctx, user.ID, vault.VaultID, mem.ID)
	if err != nil {
	...
	if _, err := agentB.AddEntryInVault(ctx, user.ID, vault.VaultID, mem.ID, client.AddEntryRequest{RawEntry: "B entry", Summary: "sum"}); err != nil {
		t.Fatalf("agentB add entry: %v", err)
	}
	_ = agentB.AwaitConsistency(ctx, mem.ID)

	entries, err := agentB.ListEntriesInVault(ctx, user.ID, vault.VaultID, mem.ID, map[string]string{"limit": "10"})
	if err != nil || len(entries.Entries) != 2 {
		t.Fatalf("expected 2 entries, got %d (%v)", len(entries.Entries), err)
	}

	// cleanup
	_ = agentB.DeleteMemory(ctx, user.ID, mem.ID)
