...
func TestAddEntryE2E(t *testing.T) {
	...
	mem, err := c.CreateMemoryInVault(ctx, user.ID, vault.VaultID, client.CreateMemoryRequest{Title: "Demo", MemoryType: "NOTES"})
	if err != nil {
		t.Fatalf("create memory: %v", err)
	}

	// set context
	_, err = c.PutMemoryContext(ctx, user.ID, vault.VaultID, mem.ID, "ctx")
	if err != nil {
	...
	// add entry
	_, err = c.AddEntryInVault(ctx, user.ID, vault.VaultID, mem.ID, client.AddEntryRequest{RawEntry: "hello", Summary: "sum"})
	if err != nil {
	...
	// list entries
	lr, err := c.ListEntriesInVault(ctx, user.ID, vault.VaultID, mem.ID, nil)
	if err != nil {
