...
import (
	...
	"github.com/synapse/synapse-mcp-server/internal/handlers"
)
...
func loadConfig() *config {
	cfg := &config{
		...
		ServerName:       getEnvOrDefault("MCP_SERVER_NAME", "synapse-mcp-server"),
...
func main() {
	...
	if shouldUseStdio() {
		...
		log.Info().Msg("Starting Synapse MCP server (stdio transport)")
		...
		log.Info().Msg("Starting Synapse MCP server (Streamable HTTP) on :11546")
