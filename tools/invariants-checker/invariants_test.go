...
func TestInvariantViolationPrevention(t *testing.T) {
    ...
    t.Run("🔒 CRITICAL: CannotBypassImmutabilityValidation", func(t *testing.T) {
        ...
        checker.makeRequest(t, "PUT",
            fmt.Sprintf("/api/users/%s/memories/%s/entries/%s", userID, memoryID, entry.EntryID),
        ...
        checker.makeRequest(t, "PUT",
            fmt.Sprintf("/api/users/%s/memories/%s/entries/%s/reset", userID, memoryID, entry.EntryID),
...
func cleanupTestUser(t *testing.T, checker *InvariantChecker, userID string) {
    ...
    checker.makeRequest(t, "DELETE", fmt.Sprintf("/api/users/%s", userID), nil, http.StatusNoContent)
}
