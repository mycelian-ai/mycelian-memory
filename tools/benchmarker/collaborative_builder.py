...
class ConversationIngestor:
    """Build Mycelian memory by replaying a conversation via SessionSimulator."""

    def __init__(
        self,
        anthropic_client: anthropic.Anthropic,
        memory_client: MycelianMemoryClient,
        *,
    ...
    async def process_conversation(
        ...
        If vault_id is provided, creates memory in that vault.
        If vault_id is None, creates a unique vault for this conversation.
        """
        # Sanitize title for memory service validation (only letters, digits, hyphens)
        # Add timestamp to ensure uniqueness across benchmark runs
        import time
        timestamp = int(time.time())
        sanitized_title = f"{benchmark_name}-Conversation-{conv.conversation_id}-{timestamp}".replace("_", "-")
