...
func (s *PostgresStorage) CreateVault(ctx context.Context, req storage.CreateVaultRequest) (*storage.Vault, error) {
    ...
    `, v.ActorID, v.VaultID.String(), v.Title, v.Description)
...
func (s *PostgresStorage) GetVault(ctx context.Context, actorID string, vaultID uuid.UUID) (*storage.Vault, error) {
	var v storage.Vault
	v.ActorID = actorID
	v.VaultID = vaultID
	row := s.db.QueryRowContext(ctx, `
        SELECT title, description, creation_time FROM vaults WHERE actor_id=$1 AND vault_id=$2
    `, actorID, vaultID.String())
...
func (s *PostgresStorage) ListVaults(ctx context.Context, actorID string) ([]*storage.Vault, error) {
...
func (s *PostgresStorage) GetVaultByTitle(ctx context.Context, actorID string, title string) (*storage.Vault, error) {
	var v storage.Vault
	v.ActorID = actorID
	v.Title = title
	row := s.db.QueryRowContext(ctx, `
...
func (s *PostgresStorage) DeleteVault(ctx context.Context, actorID string, vaultID uuid.UUID) error {
	...
	entryRows, err := tx.QueryContext(ctx, `SELECT entry_id FROM memory_entries WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String())
	if err != nil {
		return err
	}
	var entryIDs []string
	...
	ctxRows, err := tx.QueryContext(ctx, `SELECT context_id FROM memory_contexts WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String())
	if err != nil {
		return err
	}
	var ctxIDs []string
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM memory_entries WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String()); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM memory_contexts WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String()); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM memories WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String()); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM vaults WHERE actor_id=$1 AND vault_id=$2`, actorID, vaultID.String()); err != nil {
		return err
	}

	// Enqueue outbox deletes (idempotent)
	for _, id := range entryIDs {
		if err := writeOutbox(ctx, tx, "delete_entry", id, map[string]interface{}{"actorId": actorID}); err != nil {
			return err
		}
	}
	for _, id := range ctxIDs {
		if err := writeOutbox(ctx, tx, "delete_context", id, map[string]interface{}{"actorId": actorID}); err != nil {
			return err
		}
	}
...
func (s *PostgresStorage) CreateMemory(ctx context.Context, req storage.CreateMemoryRequest) (*storage.Memory, error) {
	...
	if err := tx.QueryRowContext(ctx, `
        ...
        VALUES ($1,$2,$3,$4,$5,$6)
        RETURNING creation_time
    `, mem.ActorID, mem.VaultID.String(), mem.MemoryID, mem.MemoryType, mem.Title, mem.Description).Scan(&mem.CreationTime); err != nil {
		return nil, err
	}
...
func (s *PostgresStorage) GetMemory(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string) (*storage.Memory, error) {
...
func (s *PostgresStorage) ListMemories(ctx context.Context, actorID string, vaultID uuid.UUID) ([]*storage.Memory, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *PostgresStorage) GetMemoryByTitle(ctx context.Context, actorID string, vaultID uuid.UUID, title string) (*storage.Memory, error) {
	var m storage.Memory
	m.ActorID = actorID
	m.VaultID = vaultID
	m.Title = title
...
func (s *PostgresStorage) DeleteMemory(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string) error {
	...
	entryRows, err := tx.QueryContext(ctx, `SELECT entry_id FROM memory_entries WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3`, actorID, vaultID.String(), memoryID)
	if err != nil {
		return err
	}
	var entryIDs []string
	...
	ctxRows, err := tx.QueryContext(ctx, `SELECT context_id FROM memory_contexts WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3`, actorID, vaultID.String(), memoryID)
	if err != nil {
		return err
	}
	var ctxIDs []string
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM memory_entries WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3`, actorID, vaultID.String(), memoryID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM memory_contexts WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3`, actorID, vaultID.String(), memoryID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM memories WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3`, actorID, vaultID.String(), memoryID); err != nil {
		return err
	}
...
func (s *PostgresStorage) GetMemoryEntry(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string, creationTime time.Time) (*storage.MemoryEntry, error) {
...
func (s *PostgresStorage) GetMemoryEntryByID(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string, entryID string) (*storage.MemoryEntry, error) {
	var e storage.MemoryEntry
	var vid string
	var meta, tags sql.NullString
	var corrTime, corrEntryTime, lastUpd sql.NullTime
...
func (s *PostgresStorage) ListMemoryEntries(ctx context.Context, req storage.ListMemoryEntriesRequest) ([]*storage.MemoryEntry, error) {
	...
	args := []interface{}{req.ActorID, req.VaultID.String(), req.MemoryID}
	if req.Before != nil {
...
func (s *PostgresStorage) CorrectMemoryEntry(ctx context.Context, req storage.CorrectMemoryEntryRequest) (*storage.MemoryEntry, error) {
	...
	row := tx.QueryRowContext(ctx, `
        ...
        LIMIT 1
    `, req.ActorID, req.VaultID.String(), req.MemoryID, req.OriginalCreationTime)
	...
	if _, err := tx.ExecContext(ctx, `
        ...
        SET correction_time = now(),
            ...
            correction_reason = $3
        WHERE actor_id=$4 AND vault_id=$5 AND memory_id=$6 AND creation_time=$7
    `, req.MemoryID, created, req.CorrectionReason, req.ActorID, req.VaultID.String(), req.MemoryID, req.OriginalCreationTime); err != nil {
...
func (s *PostgresStorage) UpdateMemoryEntrySummary(ctx context.Context, req storage.UpdateMemoryEntrySummaryRequest) (*storage.MemoryEntry, error) {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE memory_entries SET summary=$1, last_update_time=now() WHERE actor_id=$2 AND vault_id=$3 AND memory_id=$4 AND entry_id=$5`, req.Summary, req.ActorID, req.VaultID.String(), req.MemoryID, req.EntryID)
	if err != nil {
		return nil, err
	}
	return s.GetMemoryEntryByID(ctx, req.ActorID, req.VaultID, req.MemoryID, req.EntryID)
}
...
func (s *PostgresStorage) UpdateMemoryEntryTags(ctx context.Context, req storage.UpdateMemoryEntryTagsRequest) (*storage.MemoryEntry, error) {
	tagsJSON, _ := json.Marshal(req.Tags)
	_, err := s.db.ExecContext(ctx, `UPDATE memory_entries SET tags=$1, last_update_time=now() WHERE actor_id=$2 AND vault_id=$3 AND memory_id=$4 AND entry_id=$5`, nullIfEmpty(tagsJSON), req.ActorID, req.VaultID.String(), req.MemoryID, req.EntryID)
	if err != nil {
		return nil, err
	}
	return s.GetMemoryEntryByID(ctx, req.ActorID, req.VaultID, req.MemoryID, req.EntryID)
...
func (s *PostgresStorage) DeleteMemoryEntryByID(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string, entryID string) error {
	tx, err := s.db.BeginTx(ctx, &sql.TxOptions{})
	if err != nil {
		return err
	}
	defer func() { _ = tx.Rollback() }()
	res, err := tx.ExecContext(ctx, `DELETE FROM memory_entries WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3 AND entry_id=$4`, actorID, vaultID.String(), memoryID, entryID)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n > 0 {
		if err := writeOutbox(ctx, tx, "delete_entry", entryID, map[string]interface{}{"actorId": actorID}); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *PostgresStorage) CreateMemoryContext(ctx context.Context, req storage.CreateMemoryContextRequest) (*storage.MemoryContext, error) {
	...
	row := tx.QueryRowContext(ctx, `
        ...
        VALUES ($1,$2,$3,$4,$5)
        RETURNING creation_time
    `, req.ActorID, req.VaultID.String(), req.MemoryID, ctxID, []byte(req.Context))
	...
	return &storage.MemoryContext{ActorID: req.ActorID, VaultID: req.VaultID, MemoryID: req.MemoryID, ContextID: ctxID, Context: req.Context, CreationTime: created}, nil
}

func (s *PostgresStorage) GetLatestMemoryContext(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string) (*storage.MemoryContext, error) {
...
func (s *PostgresStorage) DeleteMemoryContextByID(ctx context.Context, actorID string, vaultID uuid.UUID, memoryID string, contextID string) error {
	tx, err := s.db.BeginTx(ctx, &sql.TxOptions{})
	if err != nil {
		return err
	}
	defer func() { _ = tx.Rollback() }()
	res, err := tx.ExecContext(ctx, `DELETE FROM memory_contexts WHERE actor_id=$1 AND vault_id=$2 AND memory_id=$3 AND context_id=$4`, actorID, vaultID.String(), memoryID, contextID)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n > 0 {
		if err := writeOutbox(ctx, tx, "delete_context", contextID, map[string]interface{}{"actorId": actorID}); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *PostgresStorage) AddMemoryToVault(ctx context.Context, req storage.AddMemoryToVaultRequest) error {
	...
	var exists int
	if err := tx.QueryRowContext(ctx, `SELECT 1 FROM vaults WHERE actor_id=$1 AND vault_id=$2`, req.ActorID, req.VaultID.String()).Scan(&exists); err != nil {
		return fmt.Errorf("VAULT_NOT_FOUND: %w", err)
	}

	// Locate current vault and title for the memory
	var currentVaultID, title string
	if err := tx.QueryRowContext(ctx, `SELECT vault_id, title FROM memories WHERE actor_id=$1 AND memory_id=$2`, req.ActorID, req.MemoryID).Scan(&currentVaultID, &title); err != nil {
	...
	var conflict int
	err = tx.QueryRowContext(ctx, `SELECT 1 FROM memories WHERE actor_id=$1 AND vault_id=$2 AND title=$3`, req.ActorID, req.VaultID.String(), title).Scan(&conflict)
	if err == nil {
	...
	// Move parent memory
	res, err := tx.ExecContext(ctx, `UPDATE memories SET vault_id=$1 WHERE actor_id=$2 AND vault_id=$3 AND memory_id=$4`, req.VaultID.String(), req.ActorID, currentVaultID, req.MemoryID)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE memory_entries SET vault_id=$1 WHERE actor_id=$2 AND vault_id=$3 AND memory_id=$4`, req.VaultID.String(), req.ActorID, currentVaultID, req.MemoryID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE memory_contexts SET vault_id=$1 WHERE actor_id=$2 AND vault_id=$3 AND memory_id=$4`, req.VaultID.String(), req.ActorID, currentVaultID, req.MemoryID); err != nil {
		return err
	}
...
func (s *PostgresStorage) DeleteMemoryFromVault(ctx context.Context, req storage.DeleteMemoryFromVaultRequest) error {
	return s.DeleteMemory(ctx, req.ActorID, req.VaultID, req.MemoryID)
}
