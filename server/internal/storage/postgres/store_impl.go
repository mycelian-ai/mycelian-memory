...
// NewWithDB returns a Postgres-backed storage.Store that uses the provided *sql.DB.
// The caller retains ownership of the database connection (lifecycle such as Close is not managed).
...
// Bootstrap performs a connectivity check against the PostgreSQL DSN and returns an error if the database cannot be reached.
// If the provided DSN is empty, Bootstrap is a no-op and returns nil.
...
// Open opens a database connection using the pgx stdlib driver.
// It validates that dsn is not empty, opens the connection, and verifies connectivity by pinging the database.
// If the ping fails the opened connection is closed.
// It returns a *sql.DB connected to the provided DSN, or an error detailing why the connection or ping failed.
...
// writeOutbox marshals the given payload to JSON and inserts an outbox record for asynchronous processing.
//
// The function uses the provided transaction and context to perform the INSERT into the outbox table.
// It returns an error if JSON marshaling or the database execution fails.
...
// nullIfEmpty returns nil when the provided byte slice is empty, otherwise it returns the slice.
// It is intended for use in SQL queries so empty JSON fields become NULL instead of an empty value.
